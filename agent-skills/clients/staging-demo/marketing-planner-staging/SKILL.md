...
### Creating a New Post
To draft a new post intended for the content calendar:
- **Action:** `POST /clients/staging-demo/posts`
- **Payload:** Minimum requires `{"title": "...", "date": "YYYY-MM-DD", "channel": "...", "status": "drafting", "copy": "..."}`.
- **Attributes:** Only include officially supported fields (e.g. `title`, `date`, `channel`, `status`, `copy`, `hashtags`, `cta`, `image`). Do **not** send arbitrary fields like `targetAudience` or the API will return `422 Unprocessable Entity`.
...
**User-provided image flow — user sends an image to post.** When the user
sends an image in chat and says "post this" (or similar), the image lands in
`/opt/data/cache/images/img_*.jpg`. Do NOT try to "see" it via browser or OCR
(those don't work reliably) — proceed with the copy/register/post flow:
1. **Copy:** `cp /opt/data/cache/images/<file> /opt/marketing-planner/client/assets/<descriptive-name>.jpg`
2. **Find next manifest ID:** Read `manifest.json`, find the highest numeric suffix across existing `id` fields (e.g. `a116` → next is `a117`), increment.
3. **Register:** Append entry to `manifest.json` items array with `source: "user-provided"`, the staging URL, and `usedInPosts: []` (will be linked after post creation).
4. **Create post:** `POST /clients/staging-demo/posts` with the staging image URL in the `image` field, `status: "in_review"`.
5. **Confirm:** Share the post ID and image URL with the user for approval.
...
- **Update Branding:** `PATCH /clients/staging-demo/branding`. This merges top-level fields (colors, typography, toneKeywords, visualGuidelines) without destroying the rest of the brief.
- **Guidelines Field:** When asked to update visual guidelines or how images should be composed, use the `visualGuidelines` property inside the branding object via the PATCH endpoint.
...
- **Branding Verification:** Before updating, use `curl` and `grep` on the client's official website (e.g., `gfinnov.com`) to extract CSS variables (`--green`, `--font-family`, etc.) and ensure hex codes match current production styles. Alternatively, read provided PDFs using `uv pip install pymupdf` and `fitz.open()` to extract brand colors, fonts, and grid guidelines.
