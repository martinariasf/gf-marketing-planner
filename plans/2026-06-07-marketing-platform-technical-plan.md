...
items:
  ...
  - gf-5: Delete Assets-section pictures | priority: medium
...
acceptance:
- Deletion behavior is defined for pictures/images shown in the Assets section: hide them through a soft-delete overlay.
...
notes:
- Source: GF-5 in Notion, clarified by Martin as the pictures/images added in the Assets section.
...
- Decision: do not remove files from disk or rewrite assets/manifest.json from the API. Dashboard deletion hides the manifest item through PocketBase overlay state.
- Technical scope: because manifest assets are Viktor-owned disk data, use an overlay before adding a delete button.
...
acceptance:
- PocketBase bootstrap includes an asset state overlay collection for Assets-section pictures.
...
acceptance:
- Assets detail dialog shows a delete control for manifest-backed pictures in the Viktor/Uploads asset folders.
