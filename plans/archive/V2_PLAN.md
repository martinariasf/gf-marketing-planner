...
- ✅ **Edit-mode for user-owned setup files.** Done 2026-05-23. Toggle in the client header flips the dashboard into edit mode; `brief.json` fields (and the same pattern is ready for `plan.json`, `goals.json`, `learnings.json`) become click-to-edit text, textareas, pills, and bullet lists. Changes live in `localStorage` as deep-partial patches per `<slug>/<file>`, so multiple sessions can accumulate edits without a backend. A floating bar shows the count of modified files and exports each one as a fully-merged, formatted JSON file ready to drop into `clients/<slug>/` and commit. Read-mode is byte-identical to before; Viktor-owned files (posts, suggestions, performance, approvals.log) remain off-limits in the UI. See `app-v2/src/lib/edit-store.tsx` + `app-v2/src/components/editable/*`.
